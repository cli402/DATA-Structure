...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int[][] random_order_v=new int[(horiz-1)*vert][3];
    int[][] random_order_h=new int[(vert-1)*horiz][3];
    
     for(int x=horiz*(vert-1);x>=0;x--){
    	random_order_h[x][0]=x;
    	random_order_h[x][1]=x/horiz;
    	random_order_h[x][2]=x%horiz;
    }
     for(int x=(horiz-1)*vert;x>=0;x--){
     	random_order_v[x][0]=x;
     	random_order_v[x][1]=x/(horiz-1);
     	random_order_v[x][2]=x%(horiz-1);
     	
     }
     
     
    int point=horiz*(vert-1);
    for(int x=horiz*(vert-1)-1;x>0;x++){
    	int choice=randInt(x);
    	int[] item=new int[3];
    	item[0]=random_order_h[choice][0];
    	item[1]=random_order_h[choice][1];
    	item[2]=random_order_h[choice][2];
    	random_order_h[choice][0]=random_order_h[point][0];
    	random_order_h[choice][1]=random_order_h[point][1];
    	random_order_h[choice][2]=random_order_h[point][2];
    	
    	random_order_h[point][0]=item[0];
    	random_order_h[point][1]=item[1];
    	random_order_h[point][2]=item[2];
    	point--;
    }
    
    int point1=(horiz-1)*vert;
    for(int x=(horiz-1)*vert-1;x>0;x++){
    	int choice=randInt(x);
    	int[] item=new int[3];
    	item[0]=random_order_v[choice][0];
    	item[1]=random_order_v[choice][1];
    	item[2]=random_order_v[choice][2];
    	random_order_v[choice][0]=random_order_v[point][0];
    	random_order_v[choice][1]=random_order_v[point][1];
    	random_order_v[choice][2]=random_order_v[point][2];
    	
    	random_order_v[point][0]=item[0];
    	random_order_v[point][1]=item[1];
    	random_order_v[point][2]=item[2];
    	point1--;
    }
    ...
    point1=(horiz-1)*vert;
	int room1,room2;
    while(point!=0 && point1!=0){
    	int choice=randInt(2);
    	if(choice==1){
    		room1=random_order_v[point1][1]*horiz+random_order_v[point1][2];
    		room2=room1+1;
    		point1--;
    		if(set.find(room1)!=set.find(room2)){
    			vWalls[random_order_v[point1][1]][random_order_v[point1][2]]=false;
    			set.union(room1, room2);
    			
    		}   		
    	}
    	else{
    		room1=random_order_h[point][1]*horiz+random_order_h[point][2];
    		room2=room1+horiz;
    		point--;
    		if(set.find(room1)!=set.find(room2)){
    			hWalls[random_order_h[point][1]][random_order_h[point][2]]=false;
    			set.union(room1, room2);
    	}
